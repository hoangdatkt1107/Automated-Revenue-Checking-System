...
def take_header(non_paylater):
    ...
    - Nếu chưa, tìm hàng chứa tất cả cột trong `expected_columns` rồi đặt làm header.
    - Không quan tâm các cột khác có hay không.
    - Nếu không tìm thấy, báo lỗi.
    """
    expected_columns = {"short name", "số lượng đơn", "phí (doanh nghiệp)", "phí (nlđ)", "phí subcription", "chiết khấu thương mại"}

    # Kiểm tra nếu header đã đúng vị trí
    current_columns = {col.lower().strip() for col in non_paylater.columns}
    if expected_columns.issubset(current_columns):  # Kiểm tra xem expected_columns có trong header không
    ...
    for i, row in non_paylater.iterrows():
        row_text = {str(cell).strip().lower() for cell in row}  # Chuyển hàng thành tập hợp (set)

        # Nếu tất cả cột trong expected_columns có trong hàng này, thì đây là header
        if expected_columns.issubset(row_text):
            header_index = i
            break  
...
def find_tenant_suggestions(df, short_names):
    """
...
def validate_company_data(non_paylater):
    company_detail = read_data_company()
    missing_company = non_paylater[non_paylater['company'].isna()]

    if not missing_company.empty:
        for _, row in missing_company.iterrows():
            short_name_value = row['Short name']  
        ...
        # Đưa ra gợi ý short name không match được với bất kì giá trị nào
            find_tenant_suggestions(company_detail,row['Short name'])

        raise ValueError("🚨 Lỗi: Có giá trị thiếu trong cột 'company'. Dừng chương trình!")
    
    print("✅ Dữ liệu hợp lệ, không có công ty nào bị thiếu thông tin.")
    return non_paylater

def fill_missing_value(df, columns, fill_value=0):
    """
    ...
    if len(columns) >= 2:
        df["accountant_revenue"] = df[columns[0]] + df[columns[1]]
        print(f"➕ Đã tạo cột 'accountant_revenue' bằng tổng của '{columns[0]}' + '{columns[1]}'")
    else:
        print("❌ Không đủ cột để cộng, cần ít nhất 2 cột!")
        raise ValueError
    df = df.groupby('company').sum('accountant_revenue').reset_index()
    return df

def take_non_paylater():
    ...
    non_paylater_folder_id, _ = find_item_in_folder(drive_service, accountant_folder_id, "non_paylater_features", mime_type="application/vnd.google-apps.folder")
    if not non_paylater_folder_id:
        return

    # Tìm file Google Sheets hoặc Excel trong "non_paylater_features"
    target_file_name = f"detail_{previous_month_folder}"
    ...
    df = take_header(df) 
    df = df[df['Short name'].notnull()]
    df['company'] = df['Short name'].apply(rename_company)
    df = validate_company_data(df)

    columns_to_fill= ["Phí (Doanh nghiệp)", "Phí (NLĐ)","Số lượng đơn","Phí subcription","Chiết khấu thương mại"]
    df = fill_missing_value(df,columns_to_fill)
